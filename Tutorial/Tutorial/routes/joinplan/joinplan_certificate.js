...
var jeremy_private_key = '0f2afa2828cf2d72b3bf3f8f7be685b117119d13381c8c696a030e1d629920d9';
var jeremy_address = '0xF1EC05328c829211733bE843385aE82eb06Bc4E9';

router.get('/', function (req, res) {

    // use index.pug
    res.render('joinplan/joinplan_certificate', {
		...
		if (err) {
			...
			connection.query('SELECT * FROM user_info WHERE user_id = ?', [req.session.user_id], function(err, rows1) {
				if (err) {
					...
					var rawTransacton = {
						"from": jeremy_address,
						"nonce": web3.toHex(web3.eth.getTransactionCount(jeremy_address)), //上面都是十六進位 use ( web3.toHex(count) = 0x2a = 42 ) repeated will nonce too low web3.toHex(web3.eth.getTransactionCount(jeremy_address))
					...
					var privateKey = Buffer.from(jeremy_private_key, "hex");
