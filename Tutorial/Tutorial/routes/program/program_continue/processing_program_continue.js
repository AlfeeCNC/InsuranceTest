...
router.post('/', function (req, res) {
    ...
    /*
    res.render('program/processing_program_pause', {
    ...
    pool.getConnection(function (err, connection) {
        connection.query('SELECT * FROM user_information WHERE user_id = ?', [req.session.user_id], function (err, rows) {
            if (err) {
                ...
                var control_id = req.body.control_id;
                var deploy_contract_address = rows[0].deploy_contract_address;

                //用不到addr_copy_trader, 所以input 給0
                var addr_copy_trader = "0x0000000000000000000000000000000000000000";
                ...
                var rawTransaction = {
                    ...
                    "gasLimit": web3.toHex(7000000),
                    "to": contractAddress,
                    "value": "0x0",
                    "data": contract.controlPreSigned.getData(sign, to, control_id, fee, nonce, deploy_contract_address, addr_copy_trader, 0, 0, "null", 0)
                };
                ...
                web3.eth.sendRawTransaction('0x' + serializedTx.toString('hex'), function (err, hash) {
                    if (!err) {
                        ...
                        return res.render('program/program_continue/processing_program_continue', {
                            date: req.body.date,
                            result: "Success",
                            tx_hash: tx_hash,
                            total_exchange: req.body.num_cash
