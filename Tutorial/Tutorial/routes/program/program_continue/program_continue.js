            ...
            connection.query('SELECT * FROM user_information WHERE user_id = ?', [req.session.user_id], function (err, rows) {
                if (err) {
                    ...
                    if (rows[0].deploy_contract_address == null) {
                        ...
                        req.flash('alert', 'You have not activated the contract');
                        return res.redirect('/program');
                    }
                    else {
                        ...
                        if (active == true) {
                            req.flash('alert', 'Your contract is continuing now');
                            return res.redirect('/program');
                        }
                        ...
                        contract.getNonce(wallet_address, function (error, nonce) {
                            if (!error) {
                                ...
                                res.render('program/program_continue/program_continue', {
