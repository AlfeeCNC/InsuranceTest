        ...
        pool.getConnection(function (err, connection) {
            connection.query('SELECT * FROM user_information WHERE user_id = ?', [req.session.user_id], function (err, rows) {
                if (err) {
                    ...
                    if (rows[0].deploy_contract_address == null) { //表示有初始化過
                        req.flash('alert', 'You have not deployed the contract');
                        return res.redirect('/program');
                    }
                    else if (!(rows[0].is_init == null || rows[0].is_init == 0)) {
                        req.flash('alert', 'You have already activated the contract');
                        return res.redirect('/program');
                    }
                    else {
                    
                        var wallet_address = rows[0].user_wallet_address;
                        var contract_address = rows[0].deploy_contract_address;
                        ...
                        contract.getNonce(wallet_address, function (error, nonce) {
                            if (!error) {
                                ...
                                res.render('program/contract_activation/contract_activation', {
