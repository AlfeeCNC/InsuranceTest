...
router.post('/', function (req, res) {
    ...
    pool.getConnection(function (err, connection) {
        pool.query('SELECT user_id FROM user_info WHERE user_id = ?', [userID], function (err, rows) {
            if (err) {
                res.render('error', {
                    message: err.message,
                    error: err
            ...
            if (rows.length >= 1) {
                pool.query('SELECT * FROM user_info WHERE user_id = ? AND user_password = ?', [userID, userPassword], function (err, rows) {
                    if (rows.length == 0) {
                        res.render('login/login');
                        console.log('�K�X���~');
                    }
